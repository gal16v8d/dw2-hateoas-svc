...
public class EvolutionController extends AbstractController<Evolution, EvolutionModel> {
  ...
  public EvolutionModel defineModelWithLinks(EvolutionModel model) {
    ...
    Link baseLink = WebMvcLinkBuilder.linkTo(
        WebMvcLinkBuilder.methodOn(DigimonController.class).getById(linkedModel.getBaseDigimonId()))
        .withRel("baseDigimon");
    Link evolvedLink = WebMvcLinkBuilder.linkTo(WebMvcLinkBuilder.methodOn(DigimonController.class)
        .getById(linkedModel.getEvolvedDigimonId())).withRel("evolvedDigimon");
