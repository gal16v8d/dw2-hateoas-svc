...
public class DigimonXAttackController {
  ...
  private DigimonXAttackModel defineModelWithLinks(DigimonXAttackModel model) {
    DigimonXAttackModel linkedModel =
        DigimonXAttackModel.builder()
            .attackId(model.getAttackId())
            .digimonId(model.getDigimonId())
            .build();
    Link selfLink =
        WebMvcLinkBuilder.linkTo(
                WebMvcLinkBuilder.methodOn(DigimonXAttackController.class)
                    .getById(linkedModel.getDigimonId(), linkedModel.getAttackId()))
            .withSelfRel();
    Link linkAttack =
        WebMvcLinkBuilder.linkTo(
                WebMvcLinkBuilder.methodOn(AttackController.class)
                    .getById(linkedModel.getAttackId()))
            .withRel("attack");
    Link linkDigimon =
        WebMvcLinkBuilder.linkTo(
                WebMvcLinkBuilder.methodOn(DigimonController.class)
                    .getById(linkedModel.getDigimonId()))
            .withRel("digimon");
  ...
  public ResponseEntity<CollectionModel<DigimonXAttackModel>> getAllAtk(
      @PathVariable("digimonId") Long digimonId) {
    List<DigimonXAttackModel> digimonXAttacks =
        digimonXAttackService.getAllAtk(digimonId).stream()
