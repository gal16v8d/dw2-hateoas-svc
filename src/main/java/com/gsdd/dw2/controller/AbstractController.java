...
public abstract class AbstractController<T, D extends RepresentationModel<D>> {
  ...
  public Link generateSelfLink(Long id) {
    try {
      Link selfLink = WebMvcLinkBuilder
          .linkTo(WebMvcLinkBuilder.methodOn(this.getClass()).getById(id)).withSelfRel();
      return selfLink;
    } catch (Exception e) {
  ...
  public ResponseEntity<CollectionModel<D>> getAll() {
    List<D> models =
        getService().getAll().stream().map(this::defineModelWithLinks).collect(Collectors.toList());
  ...
  public ResponseEntity<D> getById(@PathVariable("id") Long id) {
    return Optional.ofNullable(getService().getById(id)).map(this::defineModelWithLinks)
  ...
  public ResponseEntity<D> save(@Valid @RequestBody D model) {
    return Optional.ofNullable(getService().save(model)).map(this::defineModelWithLinks)
