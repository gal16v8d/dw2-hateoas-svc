...
public class DigimonXAttackService {
    ...
    public List<DigimonXAttackModel> getAllAtk(Long digimonId) {
        return Optional.ofNullable(digimonId)
                .map(digimonRepository::findById)
                .orElseGet(Optional::empty)
                .map(digimonXAttackRepository::findByDigimon)
    ...
    public DigimonXAttackModel associate(Long digimonId, Long attackId) {
        return Optional.ofNullable(getFromData(digimonId, attackId))
                .map(digimonXAttackConverter::convertToEntity)
                .map(digimonXAttackRepository::saveAndFlush)
                .map(digimonXAttackConverter::convertToDomain)
                .orElse(null);
    }
