...
public class AttackConverter implements GenericConverter<Attack, AttackModel> {
  ...
  public AttackModel convertToDomain(Attack entity) {
    return Optional.ofNullable(entity)
        .map(e -> AttackModel.builder().attackId(e.getAttackId())
            .attackTypeId(e.getAttackType().getAttackTypeId()).name(e.getName()).mp(e.getMp())
            .build())
        .orElse(null);
  }
  ...
  public Attack convertToEntity(AttackModel model) {
    ...
    return typeOp
        .map(t -> Optional.ofNullable(model).map(m -> Attack.builder().attackId(m.getAttackId())
            .attackType(t).name(m.getName()).mp(m.getMp()).build()).orElse(null))
        .orElse(null);
  }
