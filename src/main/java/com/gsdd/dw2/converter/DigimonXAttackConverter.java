...
public class DigimonXAttackConverter
        ...
        .map(
            e ->
                DigimonXAttackModel.builder()
                    .attackId(e.getId().getAttack().getAttackId())
                    .digimonId(e.getId().getDigimon().getDigimonId())
                    .build())
        .orElse(null);
  }
  ...
  public DigimonXAttack convertToEntity(DigimonXAttackModel model) {
    Optional<Digimon> digi =
        Optional.ofNullable(model)
            .map(DigimonXAttackModel::getDigimonId)
            .map(digimonRepository::findById)
            .orElseGet(Optional::empty);
    Optional<Attack> atk =
        Optional.ofNullable(model)
            .map(DigimonXAttackModel::getAttackId)
            .map(attackRepository::findById)
            .orElseGet(Optional::empty);
