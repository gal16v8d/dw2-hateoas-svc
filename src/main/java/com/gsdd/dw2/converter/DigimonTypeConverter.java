...
public class DigimonTypeConverter implements GenericConverter<DigimonType, DigimonTypeModel> {
  ...
  public DigimonTypeModel convertToDomain(DigimonType entity) {
    return Optional.ofNullable(entity).map(e -> DigimonTypeModel.builder()
        .digimonTypeId(e.getDigimonTypeId()).name(e.getName()).build()).orElse(null);
  }

  @Override
  public DigimonType convertToEntity(DigimonTypeModel model) {
    return Optional.ofNullable(model).map(
        m -> DigimonType.builder().digimonTypeId(m.getDigimonTypeId()).name(m.getName()).build())
        .orElse(null);
  }
