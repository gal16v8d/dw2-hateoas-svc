...
class AttackControllerTest {
  ...
  void getAllTest() throws Exception {
    BDDMockito
        .willReturn(Arrays.asList(
            AttackModel.builder().attackTypeId(1L).attackId(1L).mp(0).name(NECRO_MAGIC).build()))
  ...
  void saveTest() throws Exception {
    AttackModel model =
        AttackModel.builder().attackTypeId(1L).attackId(1L).mp(16).name(SMILEY_BOMB).build();
    BDDMockito
        .willReturn(
            AttackModel.builder().attackTypeId(1L).attackId(1L).mp(16).name(SMILEY_BOMB).build())
  ...
  void updateTest() throws Exception {
    ...
    mvc.perform(MockMvcRequestBuilders.put(V1_ATTACK_1)
  ...
  void patchTest() throws Exception {
    ...
    mvc.perform(MockMvcRequestBuilders.patch(V1_ATTACK_1)
        .contentType(MediaType.APPLICATION_JSON_VALUE).content(MAPPER.writeValueAsString(model)))
        .andExpect(MockMvcResultMatchers.status().isOk())
        .andExpect(MockMvcResultMatchers.content().contentType(APPLICATION_HAL_JSON))
        .andExpect(MockMvcResultMatchers.jsonPath(JSON_PATH_NAME).value(SMILEY_BOMB))
  ...
  void patchNotFoundTest() throws Exception {
    ...
    BDDMockito.willReturn(null).given(attackService).patch(BDDMockito.anyLong(),
        BDDMockito.any(AttackModel.class));
    mvc.perform(MockMvcRequestBuilders.patch(V1_ATTACK_1)
        .contentType(MediaType.APPLICATION_JSON_VALUE).content(MAPPER.writeValueAsString(model)))
        .andExpect(MockMvcResultMatchers.status().isNotFound());
