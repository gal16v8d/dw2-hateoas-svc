...
class EvolutionControllerTest {
  ...
  void saveTest() throws Exception {
    ...
    BDDMockito
        .willReturn(EvolutionModel.builder().baseDigimonId(1L).evolvedDigimonId(2L)
            .dnaTimes(MAX_DNA).build())
        .given(evolutionService).save(BDDMockito.any(EvolutionModel.class));
    mvc.perform(MockMvcRequestBuilders.post(V1_EVOLUTIONS)
  ...
  void updateTest() throws Exception {
    ...
    mvc.perform(MockMvcRequestBuilders.put(V1_EVOLUTIONS_1)
        .contentType(MediaType.APPLICATION_JSON_VALUE).content(MAPPER.writeValueAsString(model)))
        .andExpect(MockMvcResultMatchers.status().isOk())
        .andExpect(MockMvcResultMatchers.content().contentType(APPLICATION_HAL_JSON))
        .andExpect(MockMvcResultMatchers.jsonPath(JSON_PATH_NAME).value(MAX_DNA))
  ...
  void patchTest() throws Exception {
    ...
    mvc.perform(MockMvcRequestBuilders.patch(V1_EVOLUTIONS_1)
        .contentType(MediaType.APPLICATION_JSON_VALUE).content(MAPPER.writeValueAsString(model)))
        .andExpect(MockMvcResultMatchers.status().isOk())
        .andExpect(MockMvcResultMatchers.content().contentType(APPLICATION_HAL_JSON))
        .andExpect(MockMvcResultMatchers.jsonPath(JSON_PATH_NAME).value(MAX_DNA))
  ...
  void deleteTest() throws Exception {
    ...
    mvc.perform(MockMvcRequestBuilders.delete(V1_EVOLUTIONS_1)
