...
class ElementControllerTest {
  ...
  void getByIdTest() throws Exception {
    BDDMockito.willReturn(ElementModel.builder().elementId(1L).name(NONE).build())
        .given(elementService).getById(BDDMockito.anyLong());
    mvc.perform(
  ...
  void saveTest() throws Exception {
    ...
    BDDMockito.willReturn(ElementModel.builder().elementId(1L).name(AIR).build())
        .given(elementService).save(BDDMockito.any(ElementModel.class));
    mvc.perform(MockMvcRequestBuilders.post(V1_ELEMENTS)
  ...
  void updateTest() throws Exception {
    ElementModel model = ElementModel.builder().name(AIR).build();
    BDDMockito.willReturn(ElementModel.builder().elementId(1L).name(AIR).build())
        .given(elementService).update(BDDMockito.anyLong(), BDDMockito.any(ElementModel.class));
    mvc.perform(MockMvcRequestBuilders.put(V1_ELEMENTS_1)
        .contentType(MediaType.APPLICATION_JSON_VALUE).content(MAPPER.writeValueAsString(model)))
        .andExpect(MockMvcResultMatchers.status().isOk())
        .andExpect(MockMvcResultMatchers.content().contentType(APPLICATION_HAL_JSON))
        .andExpect(MockMvcResultMatchers.jsonPath(JSON_PATH_NAME).value(AIR))
  ...
  void patchTest() throws Exception {
    ElementModel model = ElementModel.builder().name(AIR).build();
    BDDMockito.willReturn(ElementModel.builder().elementId(1L).name(AIR).build())
        .given(elementService).patch(BDDMockito.anyLong(), BDDMockito.any(ElementModel.class));
    mvc.perform(MockMvcRequestBuilders.patch(V1_ELEMENTS_1)
        .contentType(MediaType.APPLICATION_JSON_VALUE).content(MAPPER.writeValueAsString(model)))
        .andExpect(MockMvcResultMatchers.status().isOk())
        .andExpect(MockMvcResultMatchers.content().contentType(APPLICATION_HAL_JSON))
        .andExpect(MockMvcResultMatchers.jsonPath(JSON_PATH_NAME).value(AIR))
