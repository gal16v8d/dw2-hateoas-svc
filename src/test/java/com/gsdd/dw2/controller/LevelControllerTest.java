...
import com.gsdd.dw2.model.hateoas.LevelModel;
...
class LevelControllerTest {
  ...
  void saveTest() throws Exception {
    ...
    BDDMockito.willReturn(LevelModel.builder().levelId(1L).name(MEGA_DNA).build())
        .given(levelService).save(BDDMockito.any(LevelModel.class));
  ...
  void updateTest() throws Exception {
    LevelModel model = LevelModel.builder().name(MEGA_DNA).build();
    BDDMockito.willReturn(LevelModel.builder().levelId(1L).name(MEGA_DNA).build())
        .given(levelService).update(BDDMockito.anyLong(), BDDMockito.any(LevelModel.class));
    mvc.perform(MockMvcRequestBuilders.put(V1_LEVELS_1)
  ...
  void updateNotFoundTest() throws Exception {
    LevelModel model = LevelModel.builder().name(MEGA_DNA).build();
    BDDMockito.willReturn(null).given(levelService).update(BDDMockito.anyLong(),
        BDDMockito.any(LevelModel.class));
    mvc.perform(MockMvcRequestBuilders.put(V1_LEVELS_1)
        .contentType(MediaType.APPLICATION_JSON_VALUE).content(MAPPER.writeValueAsString(model)))
  ...
  void patchTest() throws Exception {
    LevelModel model = LevelModel.builder().name(MEGA_DNA).build();
    BDDMockito.willReturn(LevelModel.builder().levelId(1L).name(MEGA_DNA).build())
        .given(levelService).patch(BDDMockito.anyLong(), BDDMockito.any(LevelModel.class));
    mvc.perform(MockMvcRequestBuilders.patch(V1_LEVELS_1)
        .contentType(MediaType.APPLICATION_JSON_VALUE).content(MAPPER.writeValueAsString(model)))
        .andExpect(MockMvcResultMatchers.status().isOk())
        .andExpect(MockMvcResultMatchers.content().contentType(APPLICATION_HAL_JSON))
        .andExpect(MockMvcResultMatchers.jsonPath(JSON_PATH_NAME).value(MEGA_DNA))
  ...
  void patchNotFoundTest() throws Exception {
    LevelModel model = LevelModel.builder().name(MEGA_DNA).build();
    BDDMockito.willReturn(null).given(levelService).patch(BDDMockito.anyLong(),
        BDDMockito.any(LevelModel.class));
    mvc.perform(MockMvcRequestBuilders.patch(V1_LEVELS_1)
        .contentType(MediaType.APPLICATION_JSON_VALUE).content(MAPPER.writeValueAsString(model)))
        .andExpect(MockMvcResultMatchers.status().isNotFound());
