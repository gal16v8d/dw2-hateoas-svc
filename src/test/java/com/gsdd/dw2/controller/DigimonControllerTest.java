...
class DigimonControllerTest {
  ...
  void getAllTest() throws Exception {
    BDDMockito.willReturn(Arrays.asList(DigimonModel.builder().digimonId(1L).elementId(1L)
        .digimonTypeId(1L).levelId(1L).name(AGUMON).build())).given(digimonService).getAll();
    mvc.perform(
  ...
  void getByIdTest() throws Exception {
    BDDMockito.willReturn(DigimonModel.builder().digimonId(1L).elementId(1L).digimonTypeId(1L)
        .levelId(1L).name(AGUMON).build()).given(digimonService).getById(BDDMockito.anyLong());
    mvc.perform(
  ...
  void saveTest() throws Exception {
    DigimonModel model = DigimonModel.builder().elementId(1L).digimonTypeId(1L).levelId(1L)
        .name(METALGREYMON).build();
    BDDMockito
        .willReturn(DigimonModel.builder().elementId(1L).digimonTypeId(1L).levelId(1L)
            .name(METALGREYMON).build())
        .given(digimonService).save(BDDMockito.any(DigimonModel.class));
    mvc.perform(MockMvcRequestBuilders.post(V1_DIGIMON)
  ...
  void updateTest() throws Exception {
    ...
    mvc.perform(MockMvcRequestBuilders.put(V1_DIGIMON_1)
        .contentType(MediaType.APPLICATION_JSON_VALUE).content(MAPPER.writeValueAsString(model)))
        .andExpect(MockMvcResultMatchers.status().isOk())
        .andExpect(MockMvcResultMatchers.content().contentType(APPLICATION_HAL_JSON))
        .andExpect(MockMvcResultMatchers.jsonPath(JSON_PATH_NAME).value(METALGREYMON))
  ...
  void patchTest() throws Exception {
    ...
    mvc.perform(MockMvcRequestBuilders.patch(V1_DIGIMON_1)
        .contentType(MediaType.APPLICATION_JSON_VALUE).content(MAPPER.writeValueAsString(model)))
        .andExpect(MockMvcResultMatchers.status().isOk())
        .andExpect(MockMvcResultMatchers.content().contentType(APPLICATION_HAL_JSON))
        .andExpect(MockMvcResultMatchers.jsonPath(JSON_PATH_NAME).value(METALGREYMON))
  ...
  void deleteTest() throws Exception {
    ...
    mvc.perform(
        MockMvcRequestBuilders.delete(V1_DIGIMON_1).contentType(MediaType.APPLICATION_JSON_VALUE))
