...
class AttackTypeControllerTest {
  ...
  void getByIdTest() throws Exception {
    BDDMockito.willReturn(AttackTypeModel.builder().attackTypeId(1L).name(ATTACK).build())
        .given(attackTypeService).getById(BDDMockito.anyLong());
    mvc.perform(
  ...
  void saveTest() throws Exception {
    ...
    BDDMockito.willReturn(AttackTypeModel.builder().attackTypeId(1L).name(RUN).build())
        .given(attackTypeService).save(BDDMockito.any(AttackTypeModel.class));
    mvc.perform(MockMvcRequestBuilders.post(V1_ATTACK_TYPES)
  ...
  void updateTest() throws Exception {
    ...
    mvc.perform(MockMvcRequestBuilders.put(V1_ATTACK_TYPES_1)
        .contentType(MediaType.APPLICATION_JSON_VALUE).content(MAPPER.writeValueAsString(model)))
        .andExpect(MockMvcResultMatchers.status().isOk())
        .andExpect(MockMvcResultMatchers.content().contentType(APPLICATION_HAL_JSON))
        .andExpect(MockMvcResultMatchers.jsonPath(JSON_PATH_NAME).value(RUN))
  ...
  void patchTest() throws Exception {
    ...
    mvc.perform(MockMvcRequestBuilders.patch(V1_ATTACK_TYPES_1)
        .contentType(MediaType.APPLICATION_JSON_VALUE).content(MAPPER.writeValueAsString(model)))
        .andExpect(MockMvcResultMatchers.status().isOk())
        .andExpect(MockMvcResultMatchers.content().contentType(APPLICATION_HAL_JSON))
        .andExpect(MockMvcResultMatchers.jsonPath(JSON_PATH_NAME).value(RUN))
  ...
  void deleteTest() throws Exception {
    ...
    mvc.perform(MockMvcRequestBuilders.delete(V1_ATTACK_TYPES_1)
