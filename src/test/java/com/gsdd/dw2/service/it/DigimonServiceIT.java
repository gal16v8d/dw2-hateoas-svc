...
class DigimonServiceIT {
  ...
  @Test
  void saveTest() {
    DigimonModel result = service.save(DigimonModel.builder().name(OFANIMON).digimonTypeId(300L)
        .elementId(200L).levelId(500L).build());
    Assertions.assertNotNull(result);
  ...
  void saveNameExistsTest() {
    Assertions.assertThrows(DataIntegrityViolationException.class, () -> service.save(DigimonModel
        .builder().name(AGUMON).digimonTypeId(300L).elementId(200L).levelId(500L).build()));
  }

  @Test
  void updateTest() {
    DigimonModel result = service.update(METALGREYMON_ID, DigimonModel.builder().name(OFANIMON)
        .digimonTypeId(300L).elementId(200L).levelId(500L).build());
    Assertions.assertNotNull(result);
    Assertions.assertEquals(OFANIMON, result.getName());
  }
  ...
  void updateNoMatchTest() {
    Assertions.assertNull(service.update(ID_NOT_FOUND, DigimonModel.builder().name(OFANIMON)
        .digimonTypeId(300L).elementId(200L).levelId(500L).build()));
  }

  @Test
  void patchTest() {
    DigimonModel result = service.patch(METALGREYMON_ID, DigimonModel.builder().name(OFANIMON)
        .digimonTypeId(300L).elementId(200L).levelId(500L).build());
    Assertions.assertNotNull(result);
    Assertions.assertEquals(OFANIMON, result.getName());
  }
  ...
  void patchNoMatchTest() {
    Assertions.assertNull(service.patch(ID_NOT_FOUND, DigimonModel.builder().name(OFANIMON)
        .digimonTypeId(300L).elementId(200L).levelId(500L).build()));
  }
