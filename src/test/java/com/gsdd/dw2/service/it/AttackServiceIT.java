...
class AttackServiceIT {
  ...
  void saveTest() {
    AttackModel result =
        service.save(AttackModel.builder().name(RUN).mp(0).attackTypeId(200L).build());
    Assertions.assertNotNull(result);
  ...
  void updateTest() {
    AttackModel result =
        service.update(
            INTERRUPT_ID, AttackModel.builder().name(RUN).mp(0).attackTypeId(200L).build());
    Assertions.assertNotNull(result);
    Assertions.assertEquals(RUN, result.getName());
  }
  ...
  void updateNoMatchTest() {
    Assertions.assertNull(
        service.update(
            ID_NOT_FOUND, AttackModel.builder().name(RUN).mp(0).attackTypeId(200L).build()));
  }
