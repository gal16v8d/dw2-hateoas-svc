...
class ElementServiceIT {
  ...
  void updateNoMatchTest() {
    Assertions.assertNull(service.update(ID_NOT_FOUND, ElementModel.builder().name(AIR).build()));
  }
  ...
  void patchNoMatchTest() {
    Assertions.assertNull(service.patch(ID_NOT_FOUND, ElementModel.builder().name(AIR).build()));
  }
